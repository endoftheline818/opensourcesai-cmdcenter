...
**The fit engine goes further: the file itself is generated.**
`src/derive/checker-engine.generated.js` is a byte-exact copy of the website's
`lib/checker-engine.js`, and it is pinned by a **sha256 of its own contents**
rather than by sampled behaviour — so the whole file is verified, not just the
values a fixture happened to probe. Never edit it; change the website and re-run
the sync. `src/derive/fit.js` is the hand-written layer above it and is where
this dashboard's own logic belongs.
