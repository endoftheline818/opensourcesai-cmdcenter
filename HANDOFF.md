# Handoff — OpenSourcesAI Command Center
...
- **Repo:** `endoftheline818/opensourcesai-cmdcenter` (GitHub, **private**)
- **Local path:** `C:\Users\cj\Documents\opensourcesai-cmdcenter`
- **Package:** `@opensourcesai/cmdcenter`, `"private": true`, **never published to npm**
...
**Status:** 129 tests passing, CI green on a 9-way matrix (ubuntu/windows/macos × Node 20/22/24). Phases 0, 1 and 2 are complete.
...
6. **Never remove `"private": true`** from `package.json`. Publishing is a founder decision. A test asserts the guard.

---

## 4. The mutation boundary (Phase 2)

Opened deliberately on founder instruction. **Widening it is a founder decision, not a refactor.**
...
There is a **hard boundary** with the website repo (`opensourcesai.com`, local path `C:\Users\cj\Documents\opensourcesai.com`). This package never imports across it. Instead it copies, and **every copy is pinned against a generated fixture**:
...
MCP config files **routinely hold live credentials** (on the reference machine: 5 servers, 7 env vars, 5 secret-shaped).
...
- **Branch and open a PR** (`feat/`, `fix/`, `docs/`, `chore/` + kebab slug). **Do not push to `main`. Do not force-push.** The founder merges.
...
- `docs/lab/2570server-test-rig.md`, `docs/lab/macbook-air-m1-test-rig.md` — the two non-Windows test machines, reachable over SSH as `2570server` and `macbook-air`.
