...
7. **An authenticating proxy at `OLLAMA_HOST` masquerades as broken Ollama.** Gateways in front of Ollama hand their clients configs that set `OLLAMA_HOST` to the proxy port — so on a machine running one, this tool can end up probing the proxy and reporting "Ollama not detected" while Ollama runs fine behind it. Bare Ollama's local API never demands credentials, so an auth-shaped refusal (401/403/407) is a specific finding: **wrong endpoint, not dead service**. Named as such in the report, the limits, and the dashboard — and the limits state the deeper reason: a proxy can route requests to a different machine, so nothing measured through one is a fact about this hardware. Finding 4's dual-purpose-variable trap, wearing a different value.
...
| site colours **and the shared instrument channel + motion** in `src/serve/ui.js` (`TOKENS`) | website `src/index.css` (16 colours × 2 themes + 5 structural) **and `src/motion.css`** (`--dur-data`, `--ease-out`) | `fixtures/website-design-tokens.json` — and since 2026-08-16 the dark half is asserted **equal** to `theme.css`'s industrial palette, not merely copied (see "The design-token contract" below) |
...
**The design-token contract (2026-08-16; website phase 4, L4; the site's rev 149).** The token pin is now a *contract*, recorded on the website side in `docs/design-tokens-contract.md`, and it has three legs that must always carry the **same name list, in the same order**: the site's `scripts/assert-css-tokens.js` (`PINNED_COLORS` / `PINNED_STRUCTURAL` / `PINNED_MOTION`), this package's `scripts/sync-from-website.mjs` (`names` / `structural` / `motion`), and the `TOKENS` mirror in `src/serve/ui.js` (a `deepEqual` in `test/design.test.js` fails if the fixture and the mirror name different sets). Two things changed in kind, not just in size: **(1) equality.** The dark half of the site's pinned palette — canvas, surface, border, muted text, primary, brand cyan, data orange, status green — is asserted **equal** to `theme.css`'s industrial palette, so the `--color-bg: var(--color-canvas)` aliases are a checked property rather than a coincidence; the one recorded non-equality is `--color-primary-hover` (a text hover on the site, which lightens on dark; a filled-control hover here, which darkens). **(2) motion.** `--dur-data` (600 ms) and `--ease-out` come from the site's `src/motion.css`; the `.metric-changed` flash animates on them and `animateLiveMetric`'s tween bakes the same number in at module scope (`DUR_DATA_MS`) — which is why the site's gate insists the token stays an integer millisecond value. The second amber this dashboard carried as a bare literal (`#f5b544`, eleven sites) is now `--metric-amber-ink`, the site's dark `--color-warn-ink`; the hairline grid paints from `--grid-line`. **Procedure per token change:** site PR edits `src/index.css` (all three blocks) or `src/motion.css` **and** its gate; this repo re-runs the sync, updates `TOKENS`, `theme.css` if a rendered value moved, and `test/design.test.js`; both merge the same day; the site's roadmap names both SHAs. A cmdcenter **release** is owed only when a rendered value moved — the L4 pair itself moved two (the flash/tween timing 620/520 → 600 ms, and the warn ink `#f5b544` → `#fbbf24`).
