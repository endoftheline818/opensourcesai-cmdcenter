// WHAT THE COMMAND LINE DOES — argument parsing, the commands themselves, and
// the wiring that boots the dashboard. Everything here imports with no side
// effects, which is what lets the suite exercise it directly.
//
// Being *run* is a separate job and belongs to exactly one file: src/cli.js,
// the executable npm installs. That split is deliberate; the reasoning is
// recorded there, and the short version is that a file which has to decide
// whether it is the entry point will eventually decide wrong — and did, twice,
// silently, once per platform.
...
// fileURLToPath, not a hand-rolled URL.pathname fix-up. The old form stripped
// the leading slash from a Windows drive letter but never percent-DECODED, so
// an install path containing a space resolved to a directory that does not
// exist (`/home/me/my apps` arrived as `/home/me/my%20apps`) and the catalog
// snapshot failed to load. `C:\Users\First Last\` is an ordinary Windows home
// directory, so that was not a corner case. Same defect family as the entry
// point bug this file was split out of: convert between URLs and paths with
// the function that knows the rules, never with string surgery.
