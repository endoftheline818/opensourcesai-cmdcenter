...
test("visual transformation charts every live counter with bounded honest history", () => {
  ...
  assert.match(
    ...
    "the twelve emitted live counters should all accumulate local history",
  );
...
test("industrial telemetry uses cyan system channels and orange hardware channels", () => {
  ...
  assert.match(js, /\["gpu", "vram", "temp", "fan", "power", "clocks", "pcie", "disk", "diskio"\]\.includes\(gauge\.id\)\) return "orange"/, "physical hardware gauges must use orange");
...
test("live system explains pressure before gauge dials", () => {
  ...
  // Both members are the machine RESPONDING to load, not the load itself: a
