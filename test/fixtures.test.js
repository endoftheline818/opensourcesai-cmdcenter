...
// `website-*` and `bench-*` are parity pins generated from companion-repo
// modules, not machine captures. Excluded by prefix rather than by an exact
// filename, so adding another parity fixture cannot accidentally make it get
// validated as a capture — one prefix per companion repo, and a parity fixture
// must carry the prefix of the repo it pins.
const PARITY_PREFIXES = ["website-", "bench-"];
const allFixtures = async () => (await readdir(fixturesDir)).filter((n) => n.endsWith(".json"));
const captureFixtures = async () =>
  (await allFixtures()).filter((n) => !PARITY_PREFIXES.some((p) => n.startsWith(p)));
...
//
// Scans EVERY fixture, parity pins included (widened 2026-08-07): a parity
// fixture is generated by executing companion-repo modules, and one that
// captured a real path — an executed module echoing its environment, say —
// would be exactly as much a leak as an unredacted capture.
test("no fixture contains a real home-directory path", async () => {
  const names = await allFixtures();
  assert.ok(names.length > 0, "no fixtures found — this guard would be vacuous");
