...
// WHAT THESE TESTS PROTECT: the VRAM gauge can say a card is 29% full, but not
// why — and "why" is the whole question when a model will not fit. The
// remainder after Ollama's own reported VRAM is the answer, and most cases
// below are ones where computing it would produce a confident wrong number.
const withVram = (usedMib, models, extra = {}) => sample({
  gpu: {
    available: true,
    gpus: [{
      index: 0, name: "NVIDIA GeForce RTX 4070 Ti",
      utilizationPercent: 5, memoryUsedMib: usedMib, memoryTotalMib: 12282,
...
// The VRAM gauge stays a reading and nothing more. A featured tile's detail is
// one nowrap ellipsised row sharing width with the big percentage, so an
// appended clause rendered as "3.5 GiB o..." on a real 1100px viewport.
test("the VRAM gauge itself carries only the reading", () => {
  const vram = buildGauges(withVram(3560, [])).find((g) => g.id === "vram");
  assert.equal(vram.detail, "3.5 / 12.0 GiB");
  assert.doesNotMatch(vram.detail, /outside Ollama/);
});

test("a trivial remainder is left unsaid rather than reported as furniture", () => {
  assert.equal(
    outside(6144, [{ name: "qwen3:4b", sizeBytes: 6 * 1024 * MIB, sizeVramBytes: 6100 * MIB }]),
    null,
    "44 MiB of rounding is not worth a chip",
  );
});
...
test("a loaded model with no size_vram makes the remainder unknowable", () => {
  assert.equal(
    outside(8192, [{ name: "qwen3:4b", sizeBytes: 7 * 1024 * MIB, sizeVramBytes: null }]),
    null,
  );
});

// The subtraction compares Ollama's total against ONE card's usage. On a
// two-GPU box that is a total against a part, and the answer is wrong in a way
...
test("two sources disagreeing beyond rounding produces silence, not a negative", () => {
  // Ollama claims 10 GiB resident while the card reports 2 GiB in use.
  const value = outside(2048, [
    { name: "phantom", sizeBytes: 10 * 1024 * MIB, sizeVramBytes: 10 * 1024 * MIB },
  ]);
